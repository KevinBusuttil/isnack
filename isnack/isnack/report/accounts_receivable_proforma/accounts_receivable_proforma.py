...
from frappe.query_builder.functions import Date, Substring, Sum, Coalesce
...
def execute(filters=None):
	...
	return ProformaReceivableReport(filters).run(args)


class ProformaReceivableReport:
	...
	def get_data(self):
		...
		# Fetch uninvoiced Sales Orders
		self.get_uninvoiced_sales_proforma()
	...
	def get_invoices(self, ple):
		if ple.voucher_type in ("Sales Invoice", "Sales Order"):
	...
	def get_voucher_balance(self, ple):
		...
		if ple.against_voucher_type in ("Sales Invoice", "Sales Order"):
	...
	def get_invoice_details(self):
		...
		if self.account_type == "Receivable":
			...
			so_list = frappe.db.sql(
				"""
				select so.name, ps.due_date, so.po_no
				from `tabSales Order` so
				join `tabPayment Schedule` ps ON ps.parent = so.name
	...
	def prepare_proforma_filter(self):
		pass
	...
	def get_uninvoiced_sales_proforma(self):
		# Simulate Sales Order ledger entries
		if self.filters.get("include_sales_orders"):  # Add a filter to control inclusion
			so = qb.DocType("Sales Order")
			ps = qb.DocType("Payment Schedule")
			pa = qb.DocType("Party Account")
			company = qb.DocType("Company")
			ifelse = CustomFunction("IF", ["condition", "then", "else"])

			so_query = (
				frappe.qb.from_(so)
				.left_join(ps).on(ps.parent == so.name)
				.left_join(pa).on((pa.parent == so.customer) & (pa.parenttype == "Customer"))
			    .left_join(company).on(company.name == so.company)  # Fetch default receivable account from Company
				.select(
					# Use customer's default account if available
					# Otherwise, use default receivable account
					#Ifnull(pa.account, company.default_receivable_account).as_("account"),
					Coalesce(pa.account, company.default_receivable_account).as_("account"),
					#company.default_receivable_account.as_("account"),
					ConstantColumn("Sales Order").as_("voucher_type"),
					so.name.as_("voucher_no"),
					ConstantColumn("Sales Order").as_("against_voucher_type"),
					so.name.as_("against_voucher_no"),
					ConstantColumn("Customer").as_("party_type"),
					ConstantColumn("").as_("cost_center"),
					so.customer.as_("party"),
					so.transaction_date.as_("posting_date"),
					ps.due_date.as_("due_date"),  # Fetch due date from Payment Schedule
					ConstantColumn("").as_("account_currency"),
					so.grand_total.as_("amount"),
					so.grand_total.as_("amount_in_account_currency")
				)
				.where(so.transaction_date.lte(self.filters.report_date))
				.where(so.docstatus == 1)
			)

			if self.filters.get("group_by_party"):
				so_query = so_query.orderby(so.customer, so.transaction_date)
			else:
				so_query = so_query.orderby(so.transaction_date, so.customer)

			so_entries = so_query.run(as_dict=True)

			# Combine PLE and Sales Order entries
			if hasattr(self, 'ple_entries') and self.ple_entries is not None:
				self.ple_entries.extend(so_entries)
			else:
				self.ple_entries = so_entries

			#Reorder the list if needed.
			if hasattr(self, 'ple_entries') and self.ple_entries is not None:
				if self.filters.get("group_by_party"):
					self.ple_entries = sorted(self.ple_entries, key=lambda x: (x.get('party'), x.get('posting_date')))
				else:
					self.ple_entries = sorted(self.ple_entries, key=lambda x: (x.get('posting_date'), x.get('party')))
		else:
			if not hasattr(self, 'ple_entries'):
				self.ple_entries = []
